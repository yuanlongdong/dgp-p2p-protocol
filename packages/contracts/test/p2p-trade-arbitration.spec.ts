...
describe("P2PTradeArbitration", function () {
  ...
  async function createAndFundTrade() {
    ...
    await fx.protocol
      .connect(fx.buyer)
      .createTrade(fx.seller.address, fx.amount, deadline);
    await fx.token
      .connect(fx.buyer)
      .approve(await fx.protocol.getAddress(), fx.amount);
  ...
  it("creates, funds, and releases a trade while updating reputation", async function () {
    const { buyer, seller, token, protocol, amount } =
      await createAndFundTrade();

    await protocol.connect(buyer).releaseTrade(1);

    expect(await token.balanceOf(seller.address)).to.equal(amount);
    expect(await protocol.getReputation(buyer.address)).to.equal(510n);
    expect(await protocol.getReputation(seller.address)).to.equal(510n);

    const trade = await protocol.getTrade(1);
    expect(trade.status).to.equal(2n); // Released
  });

  it("opens dispute, records votes, and resolves seller win", async function () {
    const { buyer, seller, arbitrator1, arbitrator2, token, protocol, amount } =
      await createAndFundTrade();
    ...
    expect(await token.balanceOf(seller.address)).to.equal(amount);
    expect(await protocol.getReputation(seller.address)).to.equal(503n);
    expect(await protocol.getReputation(buyer.address)).to.equal(492n);

    const dispute = await protocol.getDispute(1);
    expect(dispute.outcome).to.equal(2n); // SellerWin
  });
  ...
  it("resolves tie disputes in favor of refunding buyer", async function () {
    ...
    expect(await protocol.getReputation(buyer.address)).to.equal(503n);
    expect(await protocol.getReputation(seller.address)).to.equal(496n);

    const dispute = await protocol.getDispute(1);
    expect(dispute.outcome).to.equal(3n); // Tie
  });
  ...
  it("prevents duplicate votes from the same arbitrator", async function () {
    ...
    await expectRevertMessage(
      protocol.connect(arbitrator1).castVote(1, false),
      "already-voted"
    );
  });
  ...
  it("enforces vote window and disallows early dispute resolution", async function () {
    ...
    await expectRevertMessage(protocol.resolveDispute(1), "vote-active");
  });

  it("falls back to buyer refund when vote count is below configured minimum", async function () {
    const { buyer, arbitrator1, token, protocol, amount } =
      await createAndFundTrade();
    ...
    expect(await token.balanceOf(buyer.address)).to.equal(amount * 200n);
    const dispute = await protocol.getDispute(1);
    expect(dispute.outcome).to.equal(3n); // Tie fallback
  });
  ...
  it("rejects non-arbitrator vote and illegal state transitions", async function () {
    ...
    await expectRevertMessage(
      protocol.connect(outsider).releaseTrade(1),
      "only-buyer"
    );
    await protocol.connect(buyer).openDispute(1);
    await expectRevertMessage(
      protocol.connect(outsider).castVote(1, true),
      "not-arbitrator"
    );
    await expectRevertMessage(
      protocol.connect(buyer).releaseTrade(1),
      "bad-status"
    );
  });
  ...
  it("supports timeout refund path after deadline", async function () {
    ...
    expect(await token.balanceOf(buyer.address)).to.equal(amount * 200n);
    expect(await protocol.getReputation(seller.address)).to.equal(492n);
    const trade = await protocol.getTrade(1);
    expect(trade.status).to.equal(3n); // Refunded
  });

  it("keeps reputation at 0 once user reaches lower bound", async function () {
    const { buyer, seller, arbitrator1, token, protocol, amount } =
      await createAndFundTrade();
    ...
    for (let i = 0; i < 55; i++) {
      ...
      await protocol
        .connect(buyer)
        .createTrade(seller.address, amount, deadline);
